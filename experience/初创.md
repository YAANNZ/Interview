...
> 1. 通过用户登录验证和授权，获取 Access Token；
> 2. 通过 Access Token 获取用户的 OpenID；
> 3. 调用 OpenAPI，来请求访问或修改用户授权的资源。

- Core Animation
很多看似复杂酷炫的动画也是简单动画的组装，只要足够了解 Core Animation 的 API，分为一下几类：
> 1. 基础动画 （CABasicAnimation）如：位移、透明度、缩放、旋转、背景色等等
> 2. 关键帧动画（CAKeyframeAnimation）
> 3. 组动画（CAAnimationGroup）如：位移、旋转、放大为一组，可秩序进行可同时进行
> 4. 过渡动画（CATransition）转场动画，能够为层提供移出屏幕和移入屏幕的动画效果
...
> 1. leaks：用来检测内存泄漏
> 2. zombies：检测僵尸对象，（已经被释放的对象）
> 3. Allocations： 进行内存分配使用监测
> 4. Time Profiler 时间分析工具    用来检测应用CPU的使用情况.可以看到应用程序中各个方法正在消耗CPU时间
